...
function preloadActivationModeIcons()
{
    Object.values(activationModeType).forEach(modeKey =>
    {
...
function setActivationModeButtonIcon(buttonObject, bindObject)
{
    ...
    // EARLY EXIT
    if (buttonObject.dataset.mode === modeKey) return;

    buttonObject.dataset.mode = modeKey;
    buttonObject.innerHTML = '';

    // Grab from cache (preloaded)
    let baseIcon = cachedActivationModeIcons[modeKey];

    // Fallback if preload somehow missed it
    if (!baseIcon)
    {
        const img = document.createElement('img');
        img.classList.add('activation-icon');
    ...
    // Clone for insertion
    const iconClone = baseIcon.cloneNode();
